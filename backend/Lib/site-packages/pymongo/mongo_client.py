...
class MongoClient(common.BaseObject):
    ...
    def __start_session(self, implicit, **kwargs):
        # Driver Sessions Spec: "If startSession is called when multiple customusers
        # are authenticated drivers MUST raise an error with the error message
        # 'Cannot call startSession when multiple customusers are authenticated.'"
        authset = set(self.__all_credentials.values())
        if len(authset) > 1:
            raise InvalidOperation("Cannot call start_session when"
                                   " multiple customusers are authenticated")
    ...
    def _ensure_session(self, session=None):
        ...
        try:
            ...
            # Sessions not supported, or multiple customusers authenticated.
            return None
