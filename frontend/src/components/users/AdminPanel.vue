...
<!--            <p>Student groups: </p>-->
            <div v-bind:key="group.pk" v-for="group in studentGroups" class="list-item">
                <GroupElem v-bind:group="group" v-bind:toWatch="true" v-bind:isLecture="false"
                           v-bind:isStudent="false"/>
            </div>
...
<script>
    ...
    export default {
        ...
        methods: {
            saveChanges() {
                Entry.patchUserData({
                    "is_admin": this.user.is_admin, "is_lecture": this.user.is_lecture,
                    "is_student": this.user.is_student, "is_active": this.user.is_admin
                }, this.user.pk);
            }
        }
    }
